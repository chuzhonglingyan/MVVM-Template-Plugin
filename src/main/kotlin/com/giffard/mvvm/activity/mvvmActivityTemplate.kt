...
val mvvmActivityTemplate
    get() = template {
        ...
        val mModuleName = stringParameter {
            name = "Create Module Name"
            constraints = listOf(Constraint.UNIQUE, Constraint.NONEMPTY)
            default = "main"
            help = "需要生模块的名字,不需要再写 名字后缀:如Activity、Fragment,会自动生成,以及对应文件名后缀"
        }
        ...
        val mActivityLayoutName = stringParameter {
            ...
            suggest = { activityToLayout(mPageName.value.replace(Regex("([A-Z])"), "_$1")
        ...
        val mFragmentLayoutName = stringParameter {
            ...
            constraints = listOf(Constraint.LAYOUT, Constraint.NONEMPTY)
            suggest = { fragmentToLayout(mPageName.value.replace(Regex("([A-Z])"), "_$1")
                .lowercase(Locale.getDefault())) }
            visible = { mIsFragment.value }
        }
...
//        CheckBoxWidget(mIsUseHilt)
        ...
        recipe = { data: TemplateData ->
            mvvmActivityRecipe(
                ...
                mModuleName.value.lowercase(Locale.getDefault()),
                mPageName.value,
                mIsActivity.value,
